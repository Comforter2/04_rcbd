...
rcbd_dfw <- rcbd_df %>%
  ...
  mutate(trtname = paste0(nrate_kgha,"+",krate_kgha)) #adding a new column "trtname"
...
```{r rcbd_dfw}
summary(rcbd_dfw) # summary again to confirm class changes
```
...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  geom_jitter() + #jitter is a feature that makes the points offset to better see the data in case they are overlapping
...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha) #.~ helps to arrange the rows and column. It can be krate-kgha~.
  theme(legend.position = "none")
```
...
rcbd_resid <- augment(rcbd_mod) %>%
  mutate(.studresid=rstudent(rcbd_mod)) # one way to normalize our residuals to our 
  # response variable and lets us find outliers
...
- Residual normality (discuss this!), on residuals not normal data.
- Outlier detection (< -3 or > 3)  

variance error = eiidN(0, sigma2)

Better to do visual assessments than the code tests for the assumptions
...
ggplot(rcbd_resid, aes(x=.fitted, y=.studresid))+
  ...
  geom_point(shape = 21,
             ...
             alpha = .7)+
  geom_smooth()+
  geom_hline(yintercept = c(-3,3), color = "red")+ # 3 and -3 are used based on the central theorem and the standard deviation of -3 to 3  should include most of our data. Check class note
  theme_bw()
```
