...
ggplot(data = crd_layout,
  ...
  geom_text(aes(label = trtname), fontface= "italic", vjust = 1)+
  labs(title = "CRD randomization of a 3 x 3 factorial (9 treatments)") +
  theme_void()

ggsave("../output/crd_ass_layoutmap.png",
       width = 6,
       height = 4,
       bg = "white")
...
rcbd_rand = crossing(trtname = trts$trtname,
  ...
  ungroup() %>%
  mutate(plot = c(101:109, 201:209, 301:309, 401:409))
...
rcbd_rand %>%
  ...
  # Rearranging columns  
  dplyr::select(plot, rep, trtname,
                trtnum,nrate_kgha, krate_kgha) %>%
  write_csv("../output/rcbd_ass_randomization_table.csv")
...
```{r}
ggsave("../output/rcbd_ass_layoutmap.png",
       width = 6,
       height = 4,
       bg = "white")
```
